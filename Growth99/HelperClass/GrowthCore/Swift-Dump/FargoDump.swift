...
@discardableResult
public func fargoDump<T>( _ value: T,
                          name: String? = nil,
                          indent: Int = 0,
                          maxDepth: Int = .max) -> T {
    var target = ""
    let value = fargoDump(value, to: &target, name: name, indent: indent, maxDepth: maxDepth)
    print(target)
    return value
}
...
@discardableResult
public func fargoDump<T, TargetStream>(_ value: T,
                                       to target: inout TargetStream,
                                       name: String? = nil,
                                       indent: Int = 0,
                                       maxDepth: Int = .max) -> T where TargetStream: TextOutputStream {
    var visitedItems: Set<ObjectIdentifier> = []
    // swiftlint:disable all
    func fargoDumpHelp<TargetStream>(_ value: Any,
                                     to target: inout TargetStream,
                                     name: String?,
                                     indent: Int,
                                     maxDepth: Int) where TargetStream: TextOutputStream {
        let mirror = Mirror(fargoDumpReflecting: value)
        var out = ""
        
        func dumpChildren(of mirror: Mirror,
                          prefix: String,
                          suffix: String,
        ...
        case let (value as FargoDumpStringConvertible, _):
            out.write(value.fargoDumpDescription)
        case let (value as FargoDumpRepresentable, _):
            fargoDumpHelp(value.fargoDumpValue, to: &out, name: nil, indent: 0, maxDepth: maxDepth - 1)
            ...
            if mirror.children.isEmpty {
                ...
                dumpChildren(
                    ...
                    by: {
                        ...
                        return fargoDump(lhsKey.base, name: nil, indent: 0, maxDepth: 1)
                        < fargoDump(rhsKey.base, name: nil, indent: 0, maxDepth: 1)
                    }, { child, _ in
                        guard let pair = child.value as? (key: AnyHashable, value: Any) else { return }
                        let key = fargoDump(pair.key.base, name: nil, indent: 0, maxDepth: maxDepth - 1)
        ...
        case (_, .enum?):
            ...
            if let child = mirror.children.first {
                let childMirror = Mirror(fargoDumpReflecting: child.value)
                let associatedValuesMirror =
                childMirror.displayStyle == .tuple
                ? childMirror
                : Mirror(value, unlabeledChildren: [child.value], displayStyle: .tuple)
                dumpChildren( of: associatedValuesMirror, prefix: "\(child.label ?? "@unknown")(", suffix: ")", { child, _ in
        ...
        case (_, .optional?):
            if let value = mirror.children.first?.value {
                fargoDumpHelp(value, to: &out, name: nil, indent: 0, maxDepth: maxDepth)
            } else {
                out.write("nil")
            }
        case (_, .set?):
        ...
        case (_, .struct?):
            dumpChildren(of: mirror, prefix: "\(typeName(mirror.subjectType))(", suffix: ")")
        case (_, .tuple?):
            dumpChildren(of: mirror, prefix: "(", suffix: ")", { child, _ in
                if child.label?.first == "." {
                    child.label = nil
                }
            })
    ...
    fargoDumpHelp(value, to: &target, name: name, indent: indent, maxDepth: maxDepth)
    return value
}

func fargoDump(_ value: Any, name: String?, indent: Int, maxDepth: Int) -> String {
    var out = ""
    fargoDump(value, to: &out, name: name, indent: indent, maxDepth: maxDepth)
    return out
}
