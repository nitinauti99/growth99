extension AnyKeyPath: FargoDumpStringConvertible {
...
// said type.
// todo: Right now this only supports structs and class types, but in the
    ...
    var genericArgumentPointer: UnsafeRawPointer {
        ...
        case let classMetadata as ClassMetadata:
            ...
            guard !descriptor.typeFlags.classHasResilientSuperclass else {
                let memberOffset = descriptor.resilientBounds.immediateMembersOffset
...
extension ContextDescriptor {
    ...
    var flags: ContextDescriptorFlags {
        _contextDescriptor.flags
    }
}

private struct _ContextDescriptor {
    let flags: ContextDescriptorFlags
    let parent: RelativeIndirectablePointer<_ContextDescriptor>
}
...
extension TypeContextDescriptor {
    ...
    var fields: FieldDescriptor {
        ...
        let address = UnsafeRawPointer(_typeDescriptor.fields.address(from: offset))
...
private struct _TypeContextDescriptor {
    ...
    let accessor: RelativeDirectPointer<UnsafeRawPointer>
    let fields: RelativeDirectPointer<_FieldDescriptor>
}
...
private struct StructDescriptor: TypeContextDescriptor, PointerAuthenticatedLayoutWrapper {
    ...
    var fieldOffsetVectorOffset: Int {
        Int(layout.fieldOffsetVectorOffset)
    }
    // The number of stored properties this struct has.
    var numFields: Int {
        Int(layout.numFields)
    }
}

private struct _StructDescriptor {
    let base: _TypeContextDescriptor
...
private struct ClassDescriptor: TypeContextDescriptor, PointerAuthenticatedLayoutWrapper {
    ...
    var negativeSize: Int {
        ...
        return Int(layout.negativeSizeOrResilientBounds)
    }
    ...
    var positiveSize: Int {
        assert(!typeFlags.classHasResilientSuperclass)
        return Int(layout.positiveSizeOrExtraFlags)
    }
    // The resilient bounds for this class.
    var resilientBounds: _StoredClassMetadataBounds {
        let addr = address(for: \.negativeSizeOrResilientBounds)
...
private struct _ClassDescriptor {
    let base: _TypeContextDescriptor
    let superclassMangledName: RelativeDirectPointer<CChar>
    let negativeSizeOrResilientBounds: Int32
    let positiveSizeOrExtraFlags: Int32
...
private struct _StoredClassMetadataBounds {
    let immediateMembersOffset: Int
}
...
private struct FieldDescriptor: PointerAuthenticatedLayoutWrapper {
    ...
    var records: [FieldRecord] {
        ...
        for index in 0..<numFields {
            let address = trailing + MemoryLayout<_FieldRecord>.size * index
...
private struct FieldRecord: PointerAuthenticatedLayoutWrapper {
    ...
    var mangledTypeName: UnsafePointer<CChar> {
        address(for: \.mangledTypeName)
    }
    // The name of the stored property/enum case.
    var name: String {
        String(cString: address(for: \.fieldName))
    }
}

private struct _FieldDescriptor {
    let mangledTypeName: RelativeDirectPointer<CChar>
    let superclassMangledTypeName: RelativeDirectPointer<CChar>
    let kind: UInt16
...
private struct _FieldRecord {
    ...
    let mangledTypeName: RelativeDirectPointer<CChar>
    let fieldName: RelativeDirectPointer<CChar>
}
...
internal func allocObject(_: UnsafeMutableRawPointer, _: Int, _: Int) -> AnyObject?
...
private enum Reflection {
    ...
    static func allNamedKeyPaths(
        ...
        for index in 0..<metadata.contextDescriptor.fields.numFields {
            let name = metadata.contextDescriptor.fields.records[index].name
            let keyPath = createKeyPath(root: metadata, leaf: index)
