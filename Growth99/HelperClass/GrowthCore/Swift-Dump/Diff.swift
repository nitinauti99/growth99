...
///
/// FargoCore uses ``diff(_:_:format:)`` to implement ``XCTAssertNoDifference(_:_:_:file:line:)``,
/// which asserts that two values are equal, and if they are not, the failure message
/// is a nicely formatted diff showing exactly what part of the values are not equal.
///
/// - Parameters:
...
public func diff<T>(_ lhs: T, _ rhs: T, format: DiffFormat = .default) -> String? {
    ...
    func diffHelp(_ lhs: Any,
        ...
        guard !isMirrorEqual(lhs, rhs) else {
            return fargoDump(lhs, name: rhsName, indent: indent, maxDepth: 0)
                .appending(separator)
                .indenting(with: format.both + " ")
        }
        
        let lhsMirror = Mirror(fargoDumpReflecting: lhs)
        let rhsMirror = Mirror(fargoDumpReflecting: rhs)
        var out = ""
        
        func diffEverything() {
            print(
                fargoDump(lhs, name: lhsName, indent: indent, maxDepth: .max)
                    .appending(separator)
                    .indenting(with: format.first + " "),
                to: &out
            )
            print(
                fargoDump(rhs, name: rhsName, indent: indent, maxDepth: .max)
                    .appending(separator)
        ...
        func diffChildren(_ lhsMirror: Mirror,
                          _ rhsMirror: Mirror,
                          prefix: String,
                          suffix: String,
                          elementIndent: Int,
            ...
            guard !lhsMirror.children.isEmpty || !rhsMirror.children.isEmpty else {
                print(
                    fargoDump(lhs, name: lhsName, indent: indent, maxDepth: 0).indenting(with: format.first + " "),
                    to: &out
                )
                print(
                    fargoDump(rhs, name: rhsName, indent: indent, maxDepth: 0).indenting(with: format.second + " "),
                    terminator: "",
                    to: &out
                )
                return
            ...
            guard !lhsMirror.isSingleValueContainer && !rhsMirror.isSingleValueContainer else {
                print(
                    fargoDump(lhs, name: lhsName, indent: indent, maxDepth: .max).indenting(with: format.first + " "),
                    to: &out
                )
                print(
                    fargoDump(rhs, name: rhsName, indent: indent, maxDepth: .max).indenting(with: format.second + " "),
                    terminator: "",
                    to: &out
                )
                return
            ...
            print(
                name.appending(prefix).indenting(by: indent).indenting(with: format.both + " "),
                to: &out
            )
            ...
            func flushUnchanged() {
                ...
                if areInIncreasingOrder == nil && unchangedBuffer.count == 1 {
                    ...
                    print(
                        fargoDump(child.value, name: child.label, indent: indent + elementIndent, maxDepth: 0).indenting(with: format.both + " "),
                    ...
                    print(
                        "… (\(unchangedBuffer.count) unchanged)".indenting(by: indent + elementIndent).indenting(with: format.both + " "),
                        terminator: rhsOffset - 1 == rhsChildren.count - 1 ? "\n" : "\(elementSeparator)\n",
                        to: &out
                    )
                }
            ...
            while lhsOffset < lhsChildren.count || rhsOffset < rhsChildren.count {
                ...
                if collapseUnchanged, !isRemoval, !isInsertion, isMirrorEqual(lhsChildren[lhsOffset], rhsChildren[rhsOffset]) {
                ...
                case (true, false):
                    ...
                    print(
                        fargoDump(lhsChild.value, name: lhsChild.label, indent: indent + elementIndent, maxDepth: .max).indenting(with: format.first + " "),
                ...
                case (false, true):
                    ...
                    transform(&rhsChild, rhsOffset)
                    print(
                        fargoDump(rhsChild.value, name: rhsChild.label, indent: indent + elementIndent, maxDepth: .max).indenting(with: format.second + " "),
            ...
            print(
                suffix.indenting(by: indent).indenting(with: format.both + " "),
        ...
        case (is FargoDumpStringConvertible, _, is FargoDumpStringConvertible, _):
            diffEverything()
            
        case let (lhs as FargoDumpRepresentable, _, rhs as FargoDumpRepresentable, _):
            out.write(diffHelp(lhs.fargoDumpValue,
                               rhs.fargoDumpValue,
                               lhsName: lhsName,
                               rhsName: rhsName,
                               separator: separator,
        ...
        case let (lhs as AnyObject, .class?, rhs as AnyObject, .class?):
            ...
            if visitedItems.contains(lhsItem) || visitedItems.contains(rhsItem) {
                print(
                    "\(subjectType)(↩︎)".indenting(by: indent).indenting(with: format.first + " "),
                    to: &out
                )
                print(
                    "\(subjectType)(↩︎)".indenting(by: indent).indenting(with: format.second + " "),
                    terminator: "",
                    to: &out
                )
            } else {
                visitedItems.insert(lhsItem)
                diffChildren(lhsMirror,
                             rhsMirror,
                             prefix: "\(subjectType)(",
                             suffix: ")",
                             elementIndent: 2,
        ...
        case (_, .dictionary?, _, .dictionary?):
            diffChildren(lhsMirror,
                         ...
                         collapseUnchanged: true,
                         areEquivalent: {
                guard let lhs = $0.value as? (key: AnyHashable, value: Any), let rhs = $1.value as? (key: AnyHashable, value: Any) else {
            ...
            areInIncreasingOrder: {
                guard let lhs = $0.value as? (key: AnyHashable, value: Any), let rhs = $1.value as? (key: AnyHashable, value: Any) else {
                    return fargoDump($0.value, name: nil, indent: 0, maxDepth: 1) < fargoDump($1.value, name: nil, indent: 0, maxDepth: 1)
                }
                return fargoDump(lhs.key.base, name: nil, indent: 0, maxDepth: 1) < fargoDump(rhs.key.base, name: nil, indent: 0, maxDepth: 1)
            }
            ) { child, _ in
                guard let pair = child.value as? (key: AnyHashable, value: Any) else { return }
                child = ( fargoDump(pair.key.base, name: nil, indent: 0, maxDepth: 1), pair.value)
            }
            
        case (_, .enum?, _, .enum?):
            guard let lhsChild = lhsMirror.children.first,
                  let rhsChild = rhsMirror.children.first,
                  let caseName = lhsChild.label,
                  caseName == rhsChild.label else {
                      diffEverything()
                      break
                  }
            let lhsChildMirror = Mirror(fargoDumpReflecting: lhsChild.value)
            let rhsChildMirror = Mirror(fargoDumpReflecting: rhsChild.value)
            let lhsAssociatedValuesMirror =
            lhsChildMirror.displayStyle == .tuple ? lhsChildMirror : Mirror(lhs, unlabeledChildren: [lhsChild.value], displayStyle: .tuple)
            let rhsAssociatedValuesMirror = rhsChildMirror.displayStyle == .tuple ? rhsChildMirror : Mirror(rhs, unlabeledChildren: [rhsChild.value], displayStyle: .tuple)
            
            let subjectType = typeName(lhsMirror.subjectType)
            diffChildren(lhsAssociatedValuesMirror,
                         rhsAssociatedValuesMirror,
                         prefix: "\(subjectType).\(caseName)(",
                         suffix: ")",
                         elementIndent: 2,
                         elementSeparator: ",",
                         collapseUnchanged: false, { child, _ in
        ...
        case (_, .optional?, _, .optional?):
            guard let lhsValue = lhsMirror.children.first?.value, let rhsValue = rhsMirror.children.first?.value else {
                diffEverything()
                break
            }
            
            out.write(diffHelp(lhsValue,
                               rhsValue,
                               lhsName: lhsName,
                               rhsName: rhsName,
                               separator: separator,
        ...
        case (_, .set?, _, .set?):
            ...
            areInIncreasingOrder: {
                fargoDump($0.value, name: nil, indent: 0, maxDepth: 1)
                < fargoDump($1.value, name: nil, indent: 0, maxDepth: 1)
            })
        ...
        default:
            if let lhs = stringFromStringProtocol(lhs), let rhs = stringFromStringProtocol(rhs), lhs.contains(where: \.isNewline) || rhs.contains(where: \.isNewline) {
                let lhsMirror = Mirror(fargoDumpReflecting: lhs.isEmpty ? [] : lhs.split(separator: "\n", omittingEmptySubsequences: false).map(Line.init(rawValue:)))
                let rhsMirror = Mirror(fargoDumpReflecting: rhs.isEmpty ? [] : rhs.split(separator: "\n", omittingEmptySubsequences: false).map(Line.init(rawValue:)))
...
private struct Line: FargoDumpStringConvertible, Identifiable {
