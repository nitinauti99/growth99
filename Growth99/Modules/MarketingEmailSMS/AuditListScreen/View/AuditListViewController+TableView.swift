...
extension AuditListViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.delegate = self
        if isSearch {
            cell.configureCell(communicationType: communicationTypeStr, triggerModuleType: triggerModuleStr, auditFilterList: viewModel?.getAuditListFilterData[indexPath.row], index: indexPath, isSearch: isSearch)
        } else {
            cell.configureCell(communicationType: communicationTypeStr, triggerModuleType: triggerModuleStr, auditList:  viewModel?.getAuditListData[indexPath.row], index: indexPath, isSearch: isSearch)
        }
        return cell
    }
