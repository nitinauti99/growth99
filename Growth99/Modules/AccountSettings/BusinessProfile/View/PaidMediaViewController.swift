...
class PaidMediaViewController: UIViewController, UITextViewDelegate {
    ...
    func textViewDidChange(_ textView: UITextView) {
        if textView == paidMediaCodeTextView {
            guard let textField = paidMediaCodeTextView.text, !textField.isEmpty else {
                errorLabel.text = Constant.ErrorMessage.emptyURLError
                errorLabel.isHidden = false
                return
            }
    ...
    func textViewDidBeginEditing(_ textView: UITextView) {
        if textView == paidMediaCodeTextView {
            guard let textView = paidMediaCodeTextView.text, !textView.isEmpty else {
                errorLabel.text = Constant.ErrorMessage.emptyURLError
                errorLabel.isHidden = false
                return
            }
