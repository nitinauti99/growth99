...
class EmailTemplatesListTableViewCell: UITableViewCell {
    ...
    func configureCell(emailTemplateList: EmailTemplateViewModelProtocol?, index: IndexPath, selectedIndex: Int) {
        ...
        self.createdAt.text = dateFormater?.serverToLocalDateConverter(date: emailTemplateList?.createdAt ?? String.blank)
        self.updatedAt.text =  dateFormater?.serverToLocalDateConverter(date: emailTemplateList?.updatedAt ?? String.blank)
        indexPath = index
    }
